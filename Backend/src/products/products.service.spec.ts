...
describe('ProductsService', () => {
  ...
  describe('findOne', () => {
    it('returns a product when it exists and is active', async () => {
      ...
      expect(mockPrisma.product.findFirst).toHaveBeenCalledWith(
        expect.objectContaining({ where: expect.objectContaining({ id: 'prod-uuid-1', isActive: true }) }),
      )
    })
  ...
  describe('create', () => {
    it('creates a product and images in a transaction', async () => {
      ...
      const createdProduct = { ...mockProduct, images: [{ id: 'img-1', imageUrl: 'https://x.com/img.jpg' }] }
      ...
      mockPrisma.$transaction.mockImplementation(async (cb: any) => {
        const tx = {
          ...
          productImage: { createMany: jest.fn().mockResolvedValue({ count: 1 }) },
        }
        return cb(tx)
      })
      ...
      expect(mockOrgsService.resolveOrgId).toHaveBeenCalledWith('user-uuid-org')
    })
  })
  ...
  describe('remove', () => {
    ...
    it('throws BadRequestException when active reservations exist', async () => {
      ...
      await expect(service.remove('prod-uuid-1', 'user-uuid-org')).rejects.toThrow(
        BadRequestException,
      )
    })
