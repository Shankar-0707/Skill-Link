...
describe('ReservationsService', () => {
        ...
        (cb: (tx: unknown) => Promise<unknown>) => {
          const tx = {
            product: {
              findUnique: jest
                .fn()
                .mockResolvedValue({ ...mockProduct, stockQuantity: 1 }),
            },
  ...
  describe('confirm', () => {
    ...
    it('throws ForbiddenException when org does not own the product', async () => {
      ...
      await expect(
        service.confirm('res-uuid-1', 'different-user'),
  ...
  describe('markPickedUp', () => {
    ...
    it('throws BadRequestException when transitioning from PENDING (not CONFIRMED)', async () => {
      ...
      await expect(
        service.markPickedUp('res-uuid-1', 'user-uuid-cust'),
  ...
  describe('cancel', () => {
    ...
    it('throws BadRequestException when trying to cancel a PICKED_UP reservation', async () => {
      ...
      await expect(service.cancel('res-uuid-1', actor, {})).rejects.toThrow(
        BadRequestException,
      );
    });
