...
import { Loader2, Package, PlusSquare, CalendarDays, DollarSign, ChevronRight, User, AlertCircle, ShoppingBag } from "lucide-react";
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="col-span-1 lg:col-span-8 bg-white rounded-3xl p-8 shadow-sm border border-border/40">
              <div className="flex justify-between items-end mb-8">
                <div>
                  <h2 className="text-2xl font-bold text-primary tracking-tight">Catalog Analytics</h2>
            ...
            <div className="col-span-1 lg:col-span-4 flex flex-col gap-6">
              ...
              <div className="bg-primary text-white p-6 rounded-3xl flex-1 flex flex-col justify-between shadow-lg shadow-primary/20 relative overflow-hidden group">
                ...
                <div className="grid grid-cols-2 gap-3 mt-6 relative z-10">
                  ...
                  <button onClick={() => navigate('/organisation/reservations/pending')} className="bg-white/10 hover:bg-white/20 p-4 rounded-2xl flex flex-col items-center gap-2 transition-all">
              ...
              <div className="bg-white border border-border/50 p-6 rounded-3xl flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-0.5">Total Reservations</p>
                    <h2 className="text-2xl font-black text-primary tracking-tighter">{totalReservationsCount}</h2>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-0.5">Stock Value</p>
                  <h2 className="text-2xl font-black text-foreground tracking-tighter">₹{(totalValue / 1000).toFixed(1)}k</h2>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-10">
                  ...
                  reservations.slice(0, 3).map((res) => (
                    <div key={res.id} className="bg-white p-5 rounded-2xl border-l-[6px] border-primary shadow-sm hover:translate-y-[-2px] transition-all flex justify-between items-center group">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-sm font-bold text-foreground">{res.customer?.user?.name || `Customer #${res.id.substring(0,6)}`}</p>
            ...
            {/* Quick Stats Summary */}
            <div className="bg-white border border-border/50 rounded-3xl p-8 shadow-sm flex flex-col justify-center gap-8">
               <div className="flex items-center gap-6">
                  ...
                  <div>
                    <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-1">Estimated Revenue</p>
                    <p className="text-4xl font-black text-foreground tracking-tighter">₹{totalValue.toLocaleString()}</p>
                  </div>
               </div>
               
               <div className="grid grid-cols-2 gap-4">
                  <div className="p-5 bg-secondary/20 rounded-2xl border border-border/40">
                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-2">Inventory Status</p>
                    <p className="text-lg font-bold text-foreground">{totalProducts} SKUs</p>
                  </div>
                  <div className="p-5 bg-secondary/20 rounded-2xl border border-border/40">
                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-2">Total Orders</p>
                    <p className="text-lg font-bold text-foreground">{totalReservationsCount} Requests</p>
                  </div>
               </div>
            </div>
          ...
          <section className="mt-14 space-y-6">
            <div className="flex justify-between items-end px-1">
              <div>
                <h2 className="text-2xl font-bold text-foreground tracking-tight">Recent Products</h2>
                <p className="text-sm text-muted-foreground font-medium mt-1">Latest items updated in your catalog</p>
              </div>
              <button onClick={() => navigate('/organisation/products/see_all')} className="bg-primary text-white px-6 py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest hover:scale-[1.02] active:scale-[0.98] transition-all shadow-md shadow-primary/20 flex items-center gap-2">
