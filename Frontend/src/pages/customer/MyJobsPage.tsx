...
export const MyJobsPage: React.FC = () => {
  ...
  return (
    <Layout>
      ...
      {/* Filter tabs */}
      <div className="flex items-center gap-1 mb-6 bg-surface-container p-1 rounded-xl w-fit overflow-x-auto max-w-full scrollbar-hide">
        {FILTER_TABS.map(tab => (
          <button
            key={tab.value}
            onClick={() => setActiveFilter(tab.value)}
            className={`px-4 py-1.5 rounded-lg text-sm font-label font-medium transition-all whitespace-nowrap
      ...
      {filtered.length === 0 ? (
        <EmptyState
          icon="📋"
          title="No jobs here"
          description={activeFilter === 'ALL' ? "You haven't posted any jobs yet." : "No jobs found with this status."}
          action={activeFilter === 'ALL' ? { label: 'Post a New Job', onClick: () => navigate('/user/create-job') } : undefined}
        />
      ) : (
